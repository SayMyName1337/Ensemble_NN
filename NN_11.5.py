...
class ExperimentRunner:
    ...
    def run_experiment(self, noise_type, noise_range, noise_step, n_experiments=3, use_preprocessing=True):
        ...
        nn_params = self.model_builder.optimize_neural_network(
            ...
            n_trials=50, noise_type=noise_type  # Уменьшаем количество попыток для ускорения
        )
...
class NoisyDataClassificationApp:
    ...
    def show_results_table(self):
        """Отображает улучшенную таблицу с результатами экспериментов с возможностью редактирования"""
        try:
            if not self.experiment_runner.experiment_results:
            ...
            # НОВЫЙ КОД: Добавляем переменную для хранения активной ячейки редактирования
            self.current_edit = None
            
            # НОВЫЙ КОД: Обработчик редактирования ячейки
            def on_cell_double_click(event):
                ...
                # Получаем ID строки и колонки
                item_id = table.identify_row(event.y)
                column = table.identify_column(event.x)
                column_idx = int(column[1:]) - 1  # Преобразуем "#1" в 0 и т.д.
                
                # Проверяем, что это редактируемая ячейка (не тип шума или уровень)
                column_name = table["columns"][column_idx]
                if column_name in ["Тип шума", "Уровень шума"]:
                    return  # Не редактируем эти столбцы
                
                # Получаем текущее значение
                current_value = table.item(item_id, "values")[column_idx]
                
                # Создаем поле для редактирования
                edit_frame = ttk.Frame(table)
                edit_frame.place(relx=0, rely=0, anchor=tk.NW)
                ...
                entry = ttk.Entry(edit_frame, textvariable=edit_var, width=20)
                entry.pack(fill=tk.BOTH, expand=True)
                entry.focus_set()
                
                # Сохраняем информацию о текущем редактировании
                self.current_edit = {
                    "item_id": item_id,
                    "column": column,
                ...
                def finalize_edit(save=True):
                    ...
                    if save:
                        ...
                        try:
                            ...
                            item_values = list(table.item(item_id, "values"))
                            item_values[column_idx] = new_value
                            table.item(item_id, values=item_values)
                            
                            # Обновляем исходные данные в experiment_results
                            self._update_experiment_results(item_id, column_name, new_value)
                    ...
                    # Удаляем поле редактирования
                    self.current_edit["edit_frame"].destroy()
                    self.current_edit = None
                
                # Обработчик Enter и Escape
                entry.bind("<Return>", lambda e: finalize_edit(True))
                entry.bind("<Escape>", lambda e: finalize_edit(False))
                
                # Обработчик потери фокуса
                entry.bind("<FocusOut>", lambda e: finalize_edit(True))
            
            # Привязываем двойной клик к обработчику
            ...
            self.results_table = table  # Сохраняем ссылку на таблицу
            
            # Добавляем информацию о количестве строк и возможности редактирования
            info_frame = ttk.Frame(self.table_display_frame)
            info_frame.pack(fill=tk.X, padx=5, pady=2)
            
            ttk.Label(info_frame, text=f"Всего строк: {len(report_df)}").pack(side=tk.LEFT, padx=5)
            ttk.Label(info_frame, text="Совет: Дважды щелкните по ячейке для редактирования", 
    ...
    def _update_experiment_results(self, item_id, column_name, new_value):
        """Обновляет данные в experiment_results на основе отредактированного значения в таблице"""
        try:
            # Получаем индекс строки
            row_idx = self.results_table.index(item_id)
            
            # Получаем данные строки из DataFrame
            row_data = self.report_df.iloc[row_idx]
            noise_type = row_data["Тип шума"]
            noise_level = float(row_data["Уровень шума"])
            
            # Находим индекс уровня шума в массиве noise_levels
            noise_level_idx = None
            noise_levels = self.experiment_runner.experiment_results[noise_type]["noise_levels"]
            
            for i, level in enumerate(noise_levels):
                if abs(level - noise_level) < 1e-6:  # Сравнение с плавающей точкой
                    noise_level_idx = i
                    break
                    
            if noise_level_idx is None:
                raise ValueError(f"Не удалось найти уровень шума {noise_level} в данных эксперимента")
            
            # Определяем, какое поле в experiment_results нужно обновить
            field_mapping = {
                "Ансамблевая модель": "ensemble_accuracy",
                "F1-мера ансамбля": "ensemble_f1",
                "Нейронная сеть": "nn_accuracy",
                ...
                "Эффект предобработки": "preprocessing_impact"
            }
            
            if column_name in field_mapping:
                field = field_mapping[column_name]
                
                # Специальная обработка для "Эффект предобработки" (в процентах)
                is_percentage = (column_name == "Эффект предобработки")
                
                # Парсим новое значение с учетом формата
                try:
                    if " ± " in new_value:
                        mean_str, std_str = new_value.split(" ± ")
                        
                        # Удаляем символы процента, если они есть
                        if is_percentage:
                            mean_str = mean_str.replace("%", "").strip()
                            std_str = std_str.replace("%", "").strip()
                        
                        mean_val = float(mean_str)
                        std_val = float(std_str)
                        
                        # Преобразуем из процентов в доли, если это процентное значение
                        if is_percentage:
                            mean_val /= 100.0
                            std_val /= 100.0
                    else:
                        # Если формат не соответствует, берем только первое число
                        val_str = new_value.split()[0].replace("%", "").strip()
                        mean_val = float(val_str)
                        
                        # Преобразуем из процентов в доли, если это процентное значение
                        if is_percentage:
                            mean_val /= 100.0
                        
                        # Сохраняем исходное стандартное отклонение
                        std_val = self.experiment_runner.experiment_results[noise_type][field][noise_level_idx][1]
                    
                    # Обновляем значение в experiment_results
                    self.experiment_runner.experiment_results[noise_type][field][noise_level_idx] = (mean_val, std_val)
                    
                    # Обновляем DataFrame для согласованности с правильным форматированием
                    if is_percentage:
                        self.report_df.at[row_idx, column_name] = f"{mean_val*100:.2f}% ± {std_val*100:.2f}%"
                    else:
                        self.report_df.at[row_idx, column_name] = f"{mean_val:.4f} ± {std_val:.4f}"
                    
                    print(f"Обновлено значение в {noise_type}, уровень шума {noise_level}, поле {field}: ({mean_val:.4f}, {std_val:.4f})")
                    
                    return True
                except ValueError:
                    if is_percentage:
                        raise ValueError(f"Неверный формат значения. Должно быть число% или 'число% ± число%'")
                    else:
                        raise ValueError(f"Неверный формат значения. Должно быть число или 'число ± число'")
            else:
                raise ValueError(f"Поле {column_name} не может быть обновлено")
