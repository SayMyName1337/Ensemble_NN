...
class ExperimentRunner:
    ...
    def load_dataset(self, dataset_name=None, dataset_path=None):
        ...
        if self.dataset_name == 'iris':
            ...
            self.X = heart.data.values
            self.y = heart.target.astype(int).values
            self.feature_names = heart.feature_names
            self.target_names = ['Здоров', 'Болен']
            ...
            categorical_cols = adult.data.select_dtypes(include=['category', 'object']).columns
            numerical_cols = adult.data.select_dtypes(include=['number']).columns
            
            encoder = OneHotEncoder(sparse=False, handle_unknown='ignore')
            encoded_cats = encoder.fit_transform(adult.data[categorical_cols])
            
            self.X = np.hstack([adult.data[numerical_cols].values, encoded_cats])
            self.y = (adult.target == '>50K').astype(int).values
            self.feature_names = list(numerical_cols) + [f"{col}_{cat}" for i, col in enumerate(categorical_cols) 
                                                    for cat in encoder.categories_[i]]
            self.target_names = ['<=50K', '>50K']
            print(f"Загружен набор данных Adult Census Income: {self.X.shape[0]} образцов, {self.X.shape[1]} признаков, {len(np.unique(self.y))} классов")
        ...
        elif self.dataset_name == 'german_credit':
            german = fetch_openml(name='credit-g', version=1, parser='auto')
            
            # Обработка категориальных признаков
            from sklearn.preprocessing import OneHotEncoder
            categorical_cols = german.data.select_dtypes(include=['category', 'object']).columns
            numerical_cols = german.data.select_dtypes(include=['number']).columns
            
            encoder = OneHotEncoder(sparse=False, handle_unknown='ignore')
            encoded_cats = encoder.fit_transform(german.data[categorical_cols])
            
            self.X = np.hstack([german.data[numerical_cols].values, encoded_cats])
            self.y = (german.target == 'good').astype(int).values
            self.feature_names = list(numerical_cols) + [f"{col}_{cat}" for i, col in enumerate(categorical_cols) 
                                                    for cat in encoder.categories_[i]]
            self.target_names = ['bad', 'good']
            print(f"Загружен набор данных German Credit: {self.X.shape[0]} образцов, {self.X.shape[1]} признаков, {len(np.unique(self.y))} классов")
        
        elif self.dataset_name == 'abalone':
            abalone = fetch_openml(name='abalone', version=1, parser='auto')
            self.X = abalone.data.values
            
            # Преобразуем регрессию в классификацию (на 3 класса)
            y_values = abalone.target.astype(float).values
            y_bins = np.array([np.percentile(y_values, 33), np.percentile(y_values, 66)])
            self.y = np.digitize(y_values, y_bins)
            
            self.feature_names = abalone.feature_names
            self.target_names = ['Молодой', 'Средний', 'Старый']
            print(f"Загружен набор данных Abalone: {self.X.shape[0]} образцов, {self.X.shape[1]} признаков, {len(np.unique(self.y))} классов")
            ...
            self.X = biodeg.data.values
            self.y = (biodeg.target == 'RB').astype(int).values
...
class NoisyDataClassificationApp:
    ...
    def create_widgets(self):
        ...
        ttk.Radiobutton(builtin_datasets_frame, text="Adult Census", variable=self.dataset_var, value="adult").grid(row=2, column=1, sticky=tk.W, padx=5)
        ttk.Radiobutton(builtin_datasets_frame, text="Vehicle", variable=self.dataset_var, value="vehicle").grid(row=3, column=0, sticky=tk.W, padx=5)
        ttk.Radiobutton(builtin_datasets_frame, text="German Credit", variable=self.dataset_var, value="german_credit").grid(row=3, column=1, sticky=tk.W, padx=5)
        ttk.Radiobutton(builtin_datasets_frame, text="Abalone", variable=self.dataset_var, value="abalone").grid(row=4, column=0, sticky=tk.W, padx=5)
        ttk.Radiobutton(builtin_datasets_frame, text="Banknote Auth", variable=self.dataset_var, value="banknote").grid(row=4, column=1, sticky=tk.W, padx=5)
