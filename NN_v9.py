...
class ExperimentRunner:
    ...
    def load_dataset(self, dataset_name=None, dataset_path=None):
        ...
        if self.dataset_name == 'iris':
            ...
            try:
                wine_quality = fetch_openml(name='wine-quality', version=1, parser='auto')
                self.X = np.array(wine_quality.data)
                
                # Преобразуем регрессионную задачу в классификацию по качеству
                quality = wine_quality.target.astype(float)
                # Распределяем на 3 класса: низкое (<=5), среднее (6), высокое (>=7) качество
                y_class = np.zeros_like(quality, dtype=int)
                y_class[quality <= 5] = 0  # низкое качество
                y_class[(quality > 5) & (quality < 7)] = 1  # среднее качество
                y_class[quality >= 7] = 2  # высокое качество
                ...
                raise ValueError(f"Ошибка при загрузке Wine Quality: {e}")
            ...
            try:
                ...
                self.X = np.array(sonar.data)
                self.y = (sonar.target == 'M').astype(int)  # 'M' - мина, 'R' - камень
                self.feature_names = sonar.feature_names
            ...
            try:
                glass = fetch_openml(name='glass', version=1, parser='auto')
                self.X = np.array(glass.data)
                self.y = glass.target.astype(int)
                self.feature_names = glass.feature_names
                
                # Названия типов стекла
                glass_types = ['building_windows_float_processed', 
